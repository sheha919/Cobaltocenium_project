...
d=np.empty(4)
der_data = pd.DataFrame(columns=['Name', 'CoCp2 energy', 'CoCp name','CoCp energy', 'Cp name','Cp energy','BDE', 'Dipole moment', 'CoCp2 HOMO energy', 'CoCp2 LUMO energy', 'CoCp HOMO energy', 'CoCp LUMO energy',
 'Cp HOMO energy', 'Cp LUMO energy','CoCp H charge','Cp H charge','CoCp polarizability','Cp polarizability','Hardness','dCOM','rCo-O'])

cocp2_der = np.loadtxt("der.dat", dtype='str') #der,dat includes the names for all di-substituted cocp2oh  
...
#shorten names for the substituents
#update the 'der' array with new substituent name
...
#chemical notations to the substituents
#update the 'der_new' array with new substituent name
der_new=[u'N(CH\u2083)\u2082',u'OCH\u2083',u'O(CO)CH\u2083',u'P(CH\u2083)\u2082',u'S(CH\u2083)',u'Ph',u'C\u2082H\u2083',u'C\u2082H',u'CH\u2083',u'C(CH\u2083)\u2083',u'C\u2083H\u2087',
     'H','Cl','Br','F',u'(CO)NHCH\u2083',u'(CO)NH\u2082',u'(CO)OCH\u2083',u'(CO)CH\u2083','(CO)H','(CO)F','(CO)Cl',u'CF\u2083',u'CCl\u2083',u'CBr\u2083','CN',u'SOOCH\u2083',u'NO\u2082',u'SOOCF\u2083',
    'p-PhCl','o-PhCl','m-PhCl',u'o-PhCF\u2083',u'm-PhCF\u2083',u'p-PhCF\u2083',u'p-PhCH\u2083',u'p-PhOCH\u2083',u'op-Ph(CH\u2083)\u2083',u'op-Ph(OCH\u2083)\u2083',u'NC\u2085H\u2081\u2080',u'P(Ph)\u2082',u'N(Ph)\
u2082']
    ...
    for j in range(i,num):
        ...
        for indx,line in enumerate(cocp2):
            if (str('di_cocp2_'+der[i]+'_'+der[j])) in line:
                        ...
                        der_data.at[k,'Name']=str('Co(Cp-'+der_new[i]+')(Cp-'+der_new[j]+')OH')
                        #choosing the most stable orientation to dissociate the di-substituted complex into fragments 
                        if tot_ij <= tot_ji:
                                bde= (tot_ij - cocp2_energy)*627.5
                                bde_summary.write( cocp2_name +'>> cocp : '+der[i]+' '+'>> cp : '+der[j]+' '+ '>> BDE : '+str(bde)+'\n')

                                der_data.at[k,'CoCp name']=der_new[i]
                                der_data.at[k,'CoCp energy']=cocp_i
                                der_data.at[k,'Cp name']=der_new[j]
                                ...
                                der_data.at[k,'CoCp name']=der_new[j]
                                der_data.at[k,'CoCp energy']=cocp_j
                                der_data.at[k,'Cp name']=der_new[i]
            ...
            #calculating the rcom and rco-o distances
            ...
            #the distance between center of masses of two cyclopentadienyl rings 
            r_com= math.sqrt(((corx1-corx2)/5)**2 + ((cory1-cory2)/5)**2 + ((corz1-corz2)/5)**2)
            #the distance between Co atom and the O of OH anion
...
bde_summary.close()
der_data.to_excel("cobaltocenium_data.xlsx")
